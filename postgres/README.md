...
```text
chart/    The CNPG operator          (wave 30)
config/   The database cluster + its certificates   (wave 60)
tests/    Prereq gate, then a smoke test using a real Vault password
...
[External Secrets guide](../external-secrets/README.md), using
...
1. Set `enableSuperuserAccess: true` in `config/base/cluster.yaml`
